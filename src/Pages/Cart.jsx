...
function Cart() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setLoading(true)
    axios.post(`https://himalayanjava-server.onrender.com/order`,order).then((res)=>{
    ...
    <div className="h-16 w-16 flex items-center bg-cover">
      ...
      <div className='flex flex-col md:flex-row justify-between'>
        <div className='w-full md:w-[60%]'>
          ...
          <p>Product details :</p>
          <p>You have {cartItems && cartItems.length} products in your cart</p>
          </div>
      ...
      <div className='px-5 py-2 mt-10 h-[500px] bg-slate-200 w-full md:w-[35%] rounded-lg'>
        <h2 className='text-amber-950 my-5 text-lg font-bold'>Customer details</h2>
        <form onSubmit={handleSubmit}>
          <input type="text" placeholder={`Customer's name`} value={order.customer} required onChange={(e)=>{
              setOrder((prevOrder)=>({
...prevOrder,
customer: e.target.value
              }))
          }} className='bg-gray-500 w-full px-5 py-2 text-white outline-none mb-3'/>
          <input type="text" placeholder={`Card number`} value={order.cardNo} required onChange={(e)=>{
              setOrder((prevOrder)=>({
...prevOrder,
cardNo: e.target.value
              }))
          }} className='bg-gray-500 w-full px-5 py-2 text-white outline-none mb-3'/>
          <input type="email" placeholder={`Email`} value={order.email} required onChange={(e)=>{
              setOrder((prevOrder)=>({
...prevOrder,
email: e.target.value
          ...
          <input type="text" placeholder={`Province`} value={order.province} required onChange={(e)=>{
              setOrder((prevOrder)=>({
...prevOrder,
province: e.target.value
              }))
          }} className='bg-gray-500 w-full px-5 py-2 text-white outline-none mb-3'/>
          <input type="text" placeholder={`City`} value={order.city} required onChange={(e)=>{
              setOrder((prevOrder)=>({
...prevOrder,
city: e.target.value
              }))
         ...
         <div className='flex justify-center my-10'><button type="submit" disabled={loading} className='disabled:bg-gray-400 disabled:cursor-no-drop bg-gray-500 text-white py-2 rounded-lg px-5 flex items-center gap-2'>Checkout <FaLongArrowAltRight className='mt-1'/></button></div> 
        </form>
      </div>
      </div>
      </div>
